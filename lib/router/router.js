...
Router.route('/landing', {name: 'landing2'});

Router.route('/teach', {name: 'teach'});
...
Router.route('/user/:username', {
  name: 'profile',
  data: function() {
    if ((Meteor.users.find({username: this.params.username}).count() == 1) && (Meteor.isClient)) {
      return Meteor.users.findOne({username: this.params.username});
    } else if (Meteor.isClient) {
      return Meteor.users.findOne({_id: this.params.username});
    }
  }
});
...
Router.route('/surveytwo', function() {
  this.render('surveyTwo');
});
