...
// validator is deprecated; use addValidator                                                                           // 645
SimpleSchema.prototype.addValidator = SimpleSchema.prototype.validator = function(func) {                              // 646
  this._validators.push(func);                                                                                         // 647
};                                                                                                                     // 648
                                                                                                                       // 649
/**                                                                                                                    // 650
 * @method SimpleSchema.prototype.pick                                                                                 // 651
 * @param {[fields]} The list of fields to pick to instantiate the subschema                                           // 652
 * @returns {SimpleSchema} The subschema                                                                               // 653
 */                                                                                                                    // 654
SimpleSchema.prototype.pick = function(/* arguments */) {                                                              // 655
  var self = this;                                                                                                     // 656
  var args = _.toArray(arguments);                                                                                     // 657
  args.unshift(self._schema);                                                                                          // 658
                                                                                                                       // 659
  var newSchema = _.pick.apply(null, args);                                                                            // 660
  return new SimpleSchema(newSchema);                                                                                  // 661
};                                                                                                                     // 662
                                                                                                                       // 663
SimpleSchema.prototype.omit = function() {                                                                             // 664
  var self = this;                                                                                                     // 665
  var args = _.toArray(arguments);                                                                                     // 666
  args.unshift(self._schema);                                                                                          // 667
                                                                                                                       // 668
  var newSchema = _.omit.apply(null, args);                                                                            // 669
  return new SimpleSchema(newSchema);                                                                                  // 670
};                                                                                                                     // 671
                                                                                                                       // 672
                                                                                                                       // 673
/**                                                                                                                    // 674
 * @method SimpleSchema.prototype.clean                                                                                // 675
 * @param {Object} doc - Document or modifier to clean. Referenced object will be modified in place.                   // 676
 * @param {Object} [options]                                                                                           // 677
 * @param {Boolean} [options.filter=true] - Do filtering?                                                              // 678
 * @param {Boolean} [options.autoConvert=true] - Do automatic type converting?                                         // 679
 * @param {Boolean} [options.removeEmptyStrings=true] - Remove keys in normal object or $set where the value is an empty string?
 * @param {Boolean} [options.trimStrings=true] - Trim string values?                                                   // 681
 * @param {Boolean} [options.getAutoValues=true] - Inject automatic and default values?                                // 682
 * @param {Boolean} [options.isModifier=false] - Is doc a modifier object?                                             // 683
 * @param {Object} [options.extendAutoValueContext] - This object will be added to the `this` context of autoValue functions.
 * @returns {Object} The modified doc.                                                                                 // 685
 *                                                                                                                     // 686
 * Cleans a document or modifier object. By default, will filter, automatically                                        // 687
 * type convert where possible, and inject automatic/default values. Use the options                                   // 688
 * to skip one or more of these.                                                                                       // 689
 */                                                                                                                    // 690
SimpleSchema.prototype.clean = function(doc, options) {                                                                // 691
  var self = this;                                                                                                     // 692
                                                                                                                       // 693
  // By default, doc will be filtered and autoconverted                                                                // 694
  options = _.extend({                                                                                                 // 695
    filter: true,                                                                                                      // 696
    autoConvert: true,                                                                                                 // 697
    removeEmptyStrings: true,                                                                                          // 698
    trimStrings: true,                                                                                                 // 699
    getAutoValues: true,                                                                                               // 700
    isModifier: false,                                                                                                 // 701
    extendAutoValueContext: {}                                                                                         // 702
  }, options || {});                                                                                                   // 703
                                                                                                                       // 704
  // Convert $pushAll (deprecated) to $push with $each                                                                 // 705
  if ("$pushAll" in doc) {                                                                                             // 706
    console.warn("SimpleSchema.clean: $pushAll is deprecated; converting to $push with $each");                        // 707
    doc.$push = doc.$push || {};                                                                                       // 708
    for (var field in doc.$pushAll) {                                                                                  // 709
      doc.$push[field] = doc.$push[field] || {};                                                                       // 710
      doc.$push[field].$each = doc.$push[field].$each || [];                                                           // 711
      for (var i = 0, ln = doc.$pushAll[field].length; i < ln; i++) {                                                  // 712
        doc.$push[field].$each.push(doc.$pushAll[field][i]);                                                           // 713
      }                                                                                                                // 714
      delete doc.$pushAll;                                                                                             // 715
    }                                                                                                                  // 716
  }                                                                                                                    // 717
                                                                                                                       // 718
  var mDoc = new MongoObject(doc, self._blackboxKeys);                                                                 // 719
                                                                                                                       // 720
  // Clean loop                                                                                                        // 721
  if (options.filter || options.autoConvert || options.removeEmptyStrings || options.trimStrings) {                    // 722
    mDoc.forEachNode(function() {                                                                                      // 723
      var gKey = this.genericKey, p, def, val;                                                                         // 724
      if (gKey) {                                                                                                      // 725
        def = self._schema[gKey];                                                                                      // 726
        val = this.value;                                                                                              // 727
        // Filter out props if necessary; any property is OK for $unset because we want to                             // 728
        // allow conversions to remove props that have been removed from the schema.                                   // 729
        if (options.filter && this.operator !== "$unset" && !self.allowsKey(gKey)) {                                   // 730
          // XXX Special handling for $each; maybe this could be made nicer                                            // 731
          if (this.position.slice(-7) === "[$each]") {                                                                 // 732
            mDoc.removeValueForPosition(this.position.slice(0, -7));                                                   // 733
          } else {                                                                                                     // 734
            this.remove();                                                                                             // 735
          }                                                                                                            // 736
          if (SimpleSchema.debug) {                                                                                    // 737
            console.info('SimpleSchema.clean: filtered out value that would have affected key "' + gKey + '", which is not allowed by the schema');
          }                                                                                                            // 739
          return; // no reason to do more                                                                              // 740
        }                                                                                                              // 741
        if (val !== void 0) {                                                                                          // 742
          // Autoconvert values if requested and if possible                                                           // 743
          var wasAutoConverted = false;                                                                                // 744
          if (options.autoConvert && this.operator !== "$unset" && def) {                                              // 745
            var newVal = typeconvert(val, def.type);                                                                   // 746
            // trim strings                                                                                            // 747
            if (options.trimStrings && typeof newVal === "string") {                                                   // 748
              newVal = newVal.trim();                                                                                  // 749
            }                                                                                                          // 750
            if (newVal !== void 0 && newVal !== val) {                                                                 // 751
              // remove empty strings                                                                                  // 752
              if (options.removeEmptyStrings && (!this.operator || this.operator === "$set") && typeof newVal === "string" && !newVal.length) {
                // For a document, we remove any fields that are being set to an empty string                          // 754
                newVal = void 0;                                                                                       // 755
                // For a modifier, we $unset any fields that are being set to an empty string                          // 756
                if (this.operator === "$set" && this.position.match(/\[.+?\]/g).length < 2) {                          // 757
                                                                                                                       // 758
                  ...
                  mDoc.setValueForPosition(p, "");                                                                     // 760
                                                                                                                       ...
                                                                                                                       // 763
              // Change value; if undefined, will remove it                                                            // 764
              SimpleSchema.debug && console.info('SimpleSchema.clean: autoconverted value ' + val + ' from ' + typeof val + ' to ' + typeof newVal + ' for ' + gKey);
              this.updateValue(newVal);                                                                                // 766
              wasAutoConverted = true;                                                                                 // 767
            }                                                                                                          // 768
          }                                                                                                            // 769
          if (!wasAutoConverted) {                                                                                     // 770
            // trim strings                                                                                            // 771
            if (options.trimStrings && typeof val === "string" && (!def || (def && def.trim !== false))) {             // 772
              this.updateValue(val.trim());                                                                            // 773
            }                                                                                                          // 774
            // remove empty strings                                                                                    // 775
            if (options.removeEmptyStrings && (!this.operator || this.operator === "$set") && typeof val === "string" && !val.length) {
              // For a document, we remove any fields that are being set to an empty string                            // 777
              this.remove();                                                                                           // 778
              ...
              if (this.operator === "$set" && this.position.match(/\[.+?\]/g).length < 2) {                            // 780
                p = this.position.replace("$set", "$unset");                                                           // 781
                mDoc.setValueForPosition(p, "");                                                                       // 782
              }                                                                                                        // 783
            }                                                                                                          // 784
          }                                                                                                            // 785
        }                                                                                                              // 786
      }                                                                                                                // 787
    }, {endPointsOnly: false});                                                                                        // 788
  }                                                                                                                    // 789
                                                                                                                       // 790
  // Set automatic values                                                                                              // 791
  options.getAutoValues && getAutoValues.call(self, mDoc, options.isModifier, options.extendAutoValueContext);         // 792
                                                                                                                       // 793
  // Ensure we don't have any operators set to an empty object                                                         // 794
  // since MongoDB 2.6+ will throw errors.                                                                             // 795
  if (options.isModifier) {                                                                                            // 796
    for (var op in doc) {                                                                                              // 797
      if (doc.hasOwnProperty(op) && _.isEmpty(doc[op])) {                                                              // 798
        delete doc[op];                                                                                                // 799
      }                                                                                                                // 800
    }                                                                                                                  // 801
  }                                                                                                                    // 802
  ...
  return doc;                                                                                                          // 804
                                                                                                                       ...
                                                                                                                       // 806
// Returns the entire schema object or just the definition for one key                                                 // 807
// in the schema.                                                                                                      // 808
SimpleSchema.prototype.schema = function(key) {                                                                        // 809
  var self = this;                                                                                                     // 810
  // if not null or undefined (more specific)                                                                          // 811
  if (key !== null && key !== void 0) {                                                                                // 812
    return self._schema[SimpleSchema._makeGeneric(key)];                                                               // 813
  } else {                                                                                                             // 814
    return self._schema;                                                                                               // 815
  }                                                                                                                    // 816
};                                                                                                                     // 817
                                                                                                                       // 818
// Returns the evaluated definition for one key in the schema                                                          // 819
// key = non-generic key                                                                                               // 820
// [propList] = props to include in the result, for performance                                                        // 821
// [functionContext] = used for evaluating schema options that are functions                                           // 822
SimpleSchema.prototype.getDefinition = function(key, propList, functionContext) {                                      // 823
  var self = this;                                                                                                     // 824
  var defs = self.schema(key);                                                                                         // 825
  if (!defs) {                                                                                                         // 826
    return;                                                                                                            // 827
  }                                                                                                                    // 828
                                                                                                                       // 829
  if (_.isArray(propList)) {                                                                                           // 830
    defs = _.pick(defs, propList);                                                                                     // 831
  } else {                                                                                                             // 832
    defs = _.clone(defs);                                                                                              // 833
  }                                                                                                                    // 834
                                                                                                                       // 835
  // For any options that support specifying a function,                                                               // 836
  // evaluate the functions.                                                                                           // 837
  _.each(['min', 'max', 'minCount', 'maxCount', 'allowedValues', 'optional', 'label'], function (prop) {               // 838
    if (_.isFunction(defs[prop])) {                                                                                    // 839
      defs[prop] = defs[prop].call(functionContext || {});                                                             // 840
    }                                                                                                                  // 841
  });                                                                                                                  // 842
                                                                                                                       // 843
  // Inflect label if not defined                                                                                      // 844
  defs.label = defs.label || inflectedLabel(key);                                                                      // 845
                                                                                                                       // 846
  return defs;                                                                                                         // 847
};                                                                                                                     // 848
                                                                                                                       // 849
// Check if the key is a nested dot-syntax key inside of a blackbox object                                             // 850
SimpleSchema.prototype.keyIsInBlackBox = function(key) {                                                               // 851
  var self = this;                                                                                                     // 852
  var parentPath = SimpleSchema._makeGeneric(key), lastDot, def;                                                       // 853
                                                                                                                       // 854
  // Iterate the dot-syntax hierarchy until we find a key in our schema                                                // 855
  do {                                                                                                                 // 856
    lastDot = parentPath.lastIndexOf('.');                                                                             // 857
    if (lastDot !== -1) {                                                                                              // 858
      parentPath = parentPath.slice(0, lastDot); // Remove last path component                                         // 859
      def = self.getDefinition(parentPath);                                                                            // 860
    }                                                                                                                  // 861
  } while (lastDot !== -1 && !def);                                                                                    // 862
                                                                                                                       // 863
  return !!(def && def.blackbox);                                                                                      // 864
};                                                                                                                     // 865
                                                                                                                       // 866
// Use to dynamically change the schema labels.                                                                        // 867
SimpleSchema.prototype.labels = function(labels) {                                                                     // 868
  var self = this;                                                                                                     // 869
  _.each(labels, function(label, fieldName) {                                                                          // 870
    if (!_.isString(label) && !_.isFunction(label)) {                                                                  // 871
      return;                                                                                                          // 872
    }                                                                                                                  // 873
                                                                                                                       // 874
    if (!(fieldName in self._schema)) {                                                                                // 875
      return;                                                                                                          // 876
    }                                                                                                                  // 877
                                                                                                                       // 878
    self._schema[fieldName].label = label;                                                                             // 879
    self._depsLabels[fieldName] && self._depsLabels[fieldName].changed();                                              // 880
  });                                                                                                                  // 881
};                                                                                                                     // 882
                                                                                                                       // 883
// should be used to safely get a label as string                                                                      // 884
SimpleSchema.prototype.label = function(key) {                                                                         // 885
  var self = this;                                                                                                     // 886
                                                                                                                       // 887
  // Get all labels                                                                                                    // 888
  if (key === null || key === void 0) {                                                                                // 889
    var result = {};                                                                                                   // 890
    _.each(self.schema(), function(def, fieldName) {                                                                   // 891
      result[fieldName] = self.label(fieldName);                                                                       // 892
    });                                                                                                                // 893
    return result;                                                                                                     // 894
  }                                                                                                                    // 895
                                                                                                                       // 896
  // Get label for one field                                                                                           // 897
  var def = self.getDefinition(key);                                                                                   // 898
  if (def) {                                                                                                           // 899
    var genericKey = SimpleSchema._makeGeneric(key);                                                                   // 900
    self._depsLabels[genericKey] && self._depsLabels[genericKey].depend();                                             // 901
    return def.label;                                                                                                  // 902
  }                                                                                                                    // 903
                                                                                                                       // 904
  return null;                                                                                                         // 905
};                                                                                                                     // 906
...
// Global messages                                                                                                     // 908
                                                                                                                       // 909
SimpleSchema._globalMessages = {                                                                                       // 910
  required: "[label] is required",                                                                                     // 911
  minString: "[label] must be at least [min] characters",                                                              // 912
  maxString: "[label] cannot exceed [max] characters",                                                                 // 913
  minNumber: "[label] must be at least [min]",                                                                         // 914
  maxNumber: "[label] cannot exceed [max]",                                                                            // 915
  minNumberExclusive: "[label] must be greater than [min]",                                                            // 916
  maxNumberExclusive: "[label] must be less than [max]",                                                               // 917
  minDate: "[label] must be on or after [min]",                                                                        // 918
  maxDate: "[label] cannot be after [max]",                                                                            // 919
  badDate: "[label] is not a valid date",                                                                              // 920
  minCount: "You must specify at least [minCount] values",                                                             // 921
  maxCount: "You cannot specify more than [maxCount] values",                                                          // 922
  noDecimal: "[label] must be an integer",                                                                             // 923
  notAllowed: "[value] is not an allowed value",                                                                       // 924
  expectedString: "[label] must be a string",                                                                          // 925
  expectedNumber: "[label] must be a number",                                                                          // 926
  expectedBoolean: "[label] must be a boolean",                                                                        // 927
  expectedArray: "[label] must be an array",                                                                           // 928
  expectedObject: "[label] must be an object",                                                                         // 929
  expectedConstructor: "[label] must be a [type]",                                                                     // 930
  regEx: [                                                                                                             // 931
    {msg: "[label] failed regular expression validation"},                                                             // 932
    {exp: SimpleSchema.RegEx.Email, msg: "[label] must be a valid e-mail address"},                                    // 933
    {exp: SimpleSchema.RegEx.WeakEmail, msg: "[label] must be a valid e-mail address"},                                // 934
    {exp: SimpleSchema.RegEx.Domain, msg: "[label] must be a valid domain"},                                           // 935
    {exp: SimpleSchema.RegEx.WeakDomain, msg: "[label] must be a valid domain"},                                       // 936
    {exp: SimpleSchema.RegEx.IP, msg: "[label] must be a valid IPv4 or IPv6 address"},                                 // 937
    {exp: SimpleSchema.RegEx.IPv4, msg: "[label] must be a valid IPv4 address"},                                       // 938
    {exp: SimpleSchema.RegEx.IPv6, msg: "[label] must be a valid IPv6 address"},                                       // 939
    {exp: SimpleSchema.RegEx.Url, msg: "[label] must be a valid URL"},                                                 // 940
    {exp: SimpleSchema.RegEx.Id, msg: "[label] must be a valid alphanumeric ID"}                                       // 941
  ],                                                                                                                   // 942
  keyNotInSchema: "[key] is not allowed by the schema"                                                                 // 943
};                                                                                                                     // 944
                                                                                                                       // 945
SimpleSchema.messages = function(messages) {                                                                           // 946
  _.extend(SimpleSchema._globalMessages, messages);                                                                    // 947
  SimpleSchema._depsGlobalMessages.changed();                                                                          // 948
};                                                                                                                     // 949
                                                                                                                       // 950
// Schema-specific messages                                                                                            // 951
                                                                                                                       // 952
SimpleSchema.prototype.messages = function(messages) {                                                                 // 953
  var self = this;                                                                                                     // 954
  _.extend(self._messages, messages);                                                                                  // 955
  self._depsMessages.changed();                                                                                        // 956
};                                                                                                                     // 957
                                                                                                                       // 958
// Returns a string message for the given error type and key. Uses the                                                 // 959
// def and value arguments to fill in placeholders in the error messages.                                              // 960
SimpleSchema.prototype.messageForError = function(type, key, def, value) {                                             // 961
  var self = this;                                                                                                     // 962
                                                                                                                       // 963
  // We proceed even if we can't get a definition because it might be a keyNotInSchema error                           // 964
  def = def || self.getDefinition(key, ['regEx', 'label', 'minCount', 'maxCount', 'min', 'max', 'type']) || {};        // 965
                                                                                                                       // 966
  // Adjust for complex types, currently only regEx,                                                                   // 967
  // where we might have regEx.1 meaning the second                                                                    // 968
  // expression in the array.                                                                                          // 969
  var firstTypePeriod = type.indexOf("."), index = null;                                                               // 970
  if (firstTypePeriod !== -1) {                                                                                        // 971
    index = type.substring(firstTypePeriod + 1);                                                                       // 972
    index = parseInt(index, 10);                                                                                       // 973
    type = type.substring(0, firstTypePeriod);                                                                         // 974
  }                                                                                                                    // 975
                                                                                                                       // 976
  // Which regExp is it?                                                                                               // 977
  var regExpMatch;                                                                                                     // 978
  if (type === "regEx") {                                                                                              // 979
    if (index !== null && index !== void 0 && !isNaN(index)) {                                                         // 980
      regExpMatch = def.regEx[index];                                                                                  // 981
    } else {                                                                                                           // 982
      regExpMatch = def.regEx;                                                                                         // 983
    }                                                                                                                  // 984
    if (regExpMatch) {                                                                                                 // 985
      regExpMatch = regExpMatch.toString();                                                                            // 986
    }                                                                                                                  // 987
  }                                                                                                                    // 988
                                                                                                                       // 989
  // Prep some strings to be used when finding the correct message for this error                                      // 990
  var typePlusKey = type + " " + key;                                                                                  // 991
  var genericKey = SimpleSchema._makeGeneric(key);                                                                     // 992
  var typePlusGenKey = type + " " + genericKey;                                                                        // 993
                                                                                                                       // 994
  // reactively update when message templates are changed                                                              // 995
  SimpleSchema._depsGlobalMessages.depend();                                                                           // 996
  self._depsMessages.depend();                                                                                         // 997
                                                                                                                       // 998
  // Prep a function that finds the correct message for regEx errors                                                   // 999
  function findRegExError(message) {                                                                                   // 1000
    if (type !== "regEx" || !_.isArray(message)) {                                                                     // 1001
      return message;                                                                                                  // 1002
    }                                                                                                                  // 1003
    // Parse regEx messages, which are provided in a special object array format                                       // 1004
    // [{exp: RegExp, msg: "Foo"}]                                                                                     // 1005
    // Where `exp` is optional                                                                                         // 1006
                                                                                                                       // 1007
    var msgObj;                                                                                                        // 1008
    // First see if there's one where exp matches this expression                                                      // 1009
    if (regExpMatch) {                                                                                                 // 1010
      msgObj = _.find(message, function (o) {                                                                          // 1011
        return o.exp && o.exp.toString() === regExpMatch;                                                              // 1012
      });                                                                                                              // 1013
    }                                                                                                                  // 1014
                                                                                                                       // 1015
    // If not, see if there's a default message defined                                                                // 1016
    if (!msgObj) {                                                                                                     // 1017
      msgObj = _.findWhere(message, {exp: null});                                                                      // 1018
      if (!msgObj) {                                                                                                   // 1019
        msgObj = _.findWhere(message, {exp: void 0});                                                                  // 1020
      }                                                                                                                // 1021
    }                                                                                                                  // 1022
                                                                                                                       // 1023
    return msgObj ? msgObj.msg : null;                                                                                 // 1024
  }                                                                                                                    // 1025
                                                                                                                       // 1026
  // Try finding the correct message to use at various levels, from most                                               // 1027
  // specific to least specific.                                                                                       // 1028
  var message = self._messages[typePlusKey] ||                  // (1) Use schema-specific message for specific key    // 1029
                self._messages[typePlusGenKey] ||               // (2) Use schema-specific message for generic key     // 1030
                self._messages[type];                           // (3) Use schema-specific message for type            // 1031
  message = findRegExError(message);                                                                                   // 1032
                                                                                                                       // 1033
  if (!message) {                                                                                                      // 1034
    message = SimpleSchema._globalMessages[typePlusKey] ||      // (4) Use global message for specific key             // 1035
              SimpleSchema._globalMessages[typePlusGenKey] ||   // (5) Use global message for generic key              // 1036
              SimpleSchema._globalMessages[type];               // (6) Use global message for type                     // 1037
    message = findRegExError(message);                                                                                 // 1038
  }                                                                                                                    // 1039
                                                                                                                       // 1040
  if (!message) {                                                                                                      // 1041
    return "Unknown validation error";                                                                                 // 1042
  }                                                                                                                    // 1043
                                                                                                                       // 1044
  // Now replace all placeholders in the message with the correct values                                               // 1045
                                                                                                                       // 1046
  // [key]                                                                                                             // 1047
  message = message.replace("[key]", key);                                                                             // 1048
                                                                                                                       // 1049
  // [label]                                                                                                           // 1050
  // The call to self.label() establishes a reactive dependency, too                                                   // 1051
  message = message.replace("[label]", self.label(key));                                                               // 1052
                                                                                                                       // 1053
  // [minCount]                                                                                                        // 1054
  if (typeof def.minCount !== "undefined") {                                                                           // 1055
    message = message.replace("[minCount]", def.minCount);                                                             // 1056
  }                                                                                                                    // 1057
                                                                                                                       // 1058
  // [maxCount]                                                                                                        // 1059
  if (typeof def.maxCount !== "undefined") {                                                                           // 1060
    message = message.replace("[maxCount]", def.maxCount);                                                             // 1061
  }                                                                                                                    // 1062
                                                                                                                       // 1063
  // [value]                                                                                                           // 1064
  if (value !== void 0 && value !== null) {                                                                            // 1065
    message = message.replace("[value]", value.toString());                                                            // 1066
  } else {                                                                                                             // 1067
    message = message.replace("[value]", 'null');                                                                      // 1068
  }                                                                                                                    // 1069
                                                                                                                       // 1070
  // [min] and [max]                                                                                                   // 1071
  var min = def.min;                                                                                                   // 1072
  var max = def.max;                                                                                                   // 1073
  if (def.type === Date || def.type === [Date]) {                                                                      // 1074
    if (typeof min !== "undefined") {                                                                                  // 1075
      message = message.replace("[min]", Utility.dateToDateString(min));                                               // 1076
    }                                                                                                                  // 1077
    if (typeof max !== "undefined") {                                                                                  // 1078
      message = message.replace("[max]", Utility.dateToDateString(max));                                               // 1079
    }                                                                                                                  // 1080
  } else {                                                                                                             // 1081
    if (typeof min !== "undefined") {                                                                                  // 1082
      message = message.replace("[min]", min);                                                                         // 1083
    }                                                                                                                  // 1084
    if (typeof max !== "undefined") {                                                                                  // 1085
      message = message.replace("[max]", max);                                                                         // 1086
    }                                                                                                                  // 1087
  }                                                                                                                    // 1088
                                                                                                                       // 1089
  // [type]                                                                                                            // 1090
  if (def.type instanceof Function) {                                                                                  // 1091
    message = message.replace("[type]", def.type.name);                                                                // 1092
  }                                                                                                                    // 1093
                                                                                                                       // 1094
  // Now return the message                                                                                            // 1095
  return message;                                                                                                      // 1096
};                                                                                                                     // 1097
                                                                                                                       // 1098
// Returns true if key is explicitly allowed by the schema or implied                                                  // 1099
// by other explicitly allowed keys.                                                                                   // 1100
// The key string should have $ in place of any numeric array positions.                                               // 1101
SimpleSchema.prototype.allowsKey = function(key) {                                                                     // 1102
  var self = this;                                                                                                     // 1103
                                                                                                                       // 1104
  // Loop through all keys in the schema                                                                               // 1105
  return _.any(self._schemaKeys, function(schemaKey) {                                                                 // 1106
                                                                                                                       // 1107
    // If the schema key is the test key, it's allowed.                                                                // 1108
    if (schemaKey === key) {                                                                                           // 1109
      return true;                                                                                                     // 1110
    }                                                                                                                  // 1111
                                                                                                                       // 1112
    // Black box handling                                                                                              // 1113
    if (self.schema(schemaKey).blackbox === true) {                                                                    // 1114
      var kl = schemaKey.length;                                                                                       // 1115
      var compare1 = key.slice(0, kl + 2);                                                                             // 1116
      var compare2 = compare1.slice(0, -1);                                                                            // 1117
                                                                                                                       // 1118
      // If the test key is the black box key + ".$", then the test                                                    // 1119
      // key is NOT allowed because black box keys are by definition                                                   // 1120
      // only for objects, and not for arrays.                                                                         // 1121
      if (compare1 === schemaKey + '.$') {                                                                             // 1122
        return false;                                                                                                  // 1123
      }                                                                                                                // 1124
                                                                                                                       // 1125
      // Otherwise                                                                                                     // 1126
      if (compare2 === schemaKey + '.') {                                                                              // 1127
        return true;                                                                                                   // 1128
      }                                                                                                                // 1129
    }                                                                                                                  // 1130
                                                                                                                       // 1131
    return false;                                                                                                      // 1132
                                                                                                                       ...
                                                                                                                       // 1135
SimpleSchema.prototype.newContext = function() {                                                                       // 1136
  return new SimpleSchemaValidationContext(this);                                                                      // 1137
};                                                                                                                     // 1138
                                                                                                                       // 1139
// Returns all the child keys for the object identified by the generic prefix,                                         // 1140
// or all the top level keys if no prefix is supplied.                                                                 // 1141
SimpleSchema.prototype.objectKeys = function(keyPrefix) {                                                              // 1142
  var self = this;                                                                                                     // 1143
  if (!keyPrefix) {                                                                                                    // 1144
    return self._firstLevelSchemaKeys;                                                                                 // 1145
  }                                                                                                                    // 1146
  return self._objectKeys[keyPrefix + "."] || [];                                                                      // 1147
};                                                                                                                     // 1148
                                                                                                                       // 1149
SimpleSchema.prototype.validate = function (obj) {                                                                     // 1150
  var validationContext = this.newContext();                                                                           // 1151
  var isValid = validationContext.validate(obj);                                                                       // 1152
                                                                                                                       // 1153
  if (isValid) return;                                                                                                 // 1154
                                                                                                                       // 1155
  var errors = validationContext.invalidKeys().map(function (error) {                                                  // 1156
    return {                                                                                                           // 1157
      name: error.name,                                                                                                // 1158
      type: error.type,                                                                                                // 1159
      details: {                                                                                                       // 1160
        value: error.value                                                                                             // 1161
      }                                                                                                                // 1162
    };                                                                                                                 // 1163
  });                                                                                                                  // 1164
                                                                                                                       // 1165
  throw new Package['mdg:validation-error'].ValidationError(errors);                                                   // 1166
};                                                                                                                     // 1167
                                                                                                                       // 1168
SimpleSchema.prototype.validator = function () {                                                                       // 1169
  var self = this;                                                                                                     // 1170
  // XXX Could eventually accept options and customize/filter the validation a bit                                     // 1171
  // or allow automatic cleaning                                                                                       // 1172
  return function (obj) {                                                                                              // 1173
    self.validate(obj);                                                                                                // 1174
  };                                                                                                                   // 1175
};                                                                                                                     // 1176
                                                                                                                       // 1177
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
