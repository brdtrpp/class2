      ...
      Meteor.publish(collectionName, function (searchDefinition, options) {                                          // 157
        ...
        var cursor = collectionScope.engine.search(searchDefinition, {                                               // 173
          ...
          index: collectionScope._indexConfiguration                                                                 // 175
                                                                                                                     ...
                                                                                                                     //
        this.added(collectionName, 'searchCount' + definitionString, { count: cursor.count() });                     // 178
                                                                                                                     //
        var resultsHandle = cursor.mongoCursor.observe({                                                             // 180
          addedAt: function (doc, atIndex, before) {                                                                 // 181
            doc = collectionScope.engine.config.beforePublish('addedAt', doc, atIndex, before);                      // 182
            doc = collectionScope.addCustomFields(doc, {                                                             // 183
              searchDefinition: definitionString,                                                                    // 184
              searchOptions: optionsString,                                                                          // 185
              sortPosition: atIndex,                                                                                 // 186
              originalId: doc._id                                                                                    // 187
            });                                                                                                      //
                                                                                                                     //
            _this2.added(collectionName, collectionScope.generateId(doc), doc);                                      // 190
          },                                                                                                         //
          changedAt: function (doc, oldDoc, atIndex) {                                                               // 192
            doc = collectionScope.engine.config.beforePublish('changedAt', doc, oldDoc, atIndex);                    // 193
            doc = collectionScope.addCustomFields(doc, {                                                             // 194
              searchDefinition: definitionString,                                                                    // 195
              searchOptions: optionsString,                                                                          // 196
              sortPosition: atIndex,                                                                                 // 197
              originalId: doc._id                                                                                    // 198
            });                                                                                                      //
                                                                                                                     //
            _this2.changed(collectionName, collectionScope.generateId(doc), doc);                                    // 201
          },                                                                                                         //
          movedTo: function (doc, fromIndex, toIndex, before) {                                                      // 203
            doc = collectionScope.engine.config.beforePublish('movedTo', doc, fromIndex, toIndex, before);           // 204
            doc = collectionScope.addCustomFields(doc, {                                                             // 205
              searchDefinition: definitionString,                                                                    // 206
              searchOptions: optionsString,                                                                          // 207
              sortPosition: toIndex                                                                                  // 208
            });                                                                                                      //
                                                                                                                     //
            var beforeDoc = collectionScope._indexConfiguration.collection.findOne(before);                          // 211
                                                                                                                     //
            if (beforeDoc) {                                                                                         // 213
              beforeDoc = collectionScope.addCustomFields(beforeDoc, {                                               // 214
                searchDefinition: definitionString,                                                                  // 215
                searchOptions: optionsString,                                                                        // 216
                sortPosition: fromIndex                                                                              // 217
              });                                                                                                    //
              _this2.changed(collectionName, collectionScope.generateId(beforeDoc), beforeDoc);                      // 219
            }                                                                                                        //
                                                                                                                     //
            _this2.changed(collectionName, collectionScope.generateId(doc), doc);                                    // 222
          },                                                                                                         //
          removedAt: function (doc, atIndex) {                                                                       // 224
            doc = collectionScope.engine.config.beforePublish('removedAt', doc, atIndex);                            // 225
            doc = collectionScope.addCustomFields(doc, { searchDefinition: definitionString, searchOptions: optionsString });
            _this2.removed(collectionName, collectionScope.generateId(doc));                                         // 227
          }                                                                                                          //
        });                                                                                                          //
                                                                                                                     //
        this.onStop(function () {                                                                                    // 231
          resultsHandle.stop();                                                                                      // 232
        });                                                                                                          //
                                                                                                                     //
        this.ready();                                                                                                // 235
      });                                                                                                            //
