Meteor.methods({
  recurLessonInterval:function(doc){
    var date = moment(doc.start).format("MM-DD-YYYY");
    var recur = moment(date).recur().every(doc.recur.intervalNumber, doc.recur.intervalType);
    // console.log(date);

    //Recurring individual events
    for (
      i = 0;
      i < 365;
      i++, date = JSON.stringify(moment(moment(date).add(1, "days")._d).format("MM-DD-YYYY"))
      ) {
          if (recur.matches(date) == true) {
            CalEvent.insert({title: doc.title, start: moment(date).format("YYYY-MM-DDTHH:mm:ssZ")});
          }
        }
  },

  saveCalEvent:function(ce){
    return  CalEvent.insert(ce);
  },

  moveEvent:function(event){
    return CalEvent.update({_id:event._id},{
      $set:{
        start: event.start.format(),
        end: event.end.format(),
        owner: Meteor.userId(),
      }
    });
  },
});